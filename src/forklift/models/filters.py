...
class PromisingForksFilter(BaseModel):
    ...
    max_fork_age_days: Optional[int] = Field(default=None, ge=1, description="Maximum fork age in days")
    
    @field_validator("max_fork_age_days")
    @classmethod
    def validate_max_fork_age(cls, v: Optional[int], info) -> Optional[int]:
    ...
    def _calculate_activity_score(self, activity_status: str, pushed_at: Optional[datetime]) -> float:
