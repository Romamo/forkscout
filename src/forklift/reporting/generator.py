...
from ..models.github import Commit, Fork, Repository
...
class ReportGenerator:
    ...
    def generate_analysis_report(
        ...
        report_sections.append(self._generate_overview(repository, fork_analyses, ranked_features))
        ...
        report_sections.append(self._generate_detailed_features(ranked_features[:self.max_features]))
...
"""
    ...
    def _generate_features_by_category(self, ranked_features: List[RankedFeature]) -> str:
        ...
        sorted_categories = sorted(
            categories.items(),
            key=lambda x: sum(f.score for f in x[1]),
            reverse=True
        )
        ...
        for category, features in sorted_categories:
            ...
            for feature in features[:3]:
                section += f"- **{feature.feature.title}** (Score: {feature.score:.1f})\n"
            ...
            if len(features) > 3:
                section += f"  *...and {len(features) - 3} more features in this category*\n\n"
        ...
        for i, ranked_feature in enumerate(top_features, 1):
            ...
            if feature.commits:
                ...
                for commit in feature.commits[:3]:  # Show top 3 commits
                    commit_url = f"{feature.source_fork.repository.html_url}/commit/{commit.sha}"
            ...
            if feature.files_affected:
                ...
                if len(feature.files_affected) > 5:
                    section += f"- *...and {len(feature.files_affected) - 5} more files*\n"
                section += "\n"
            ...
            if ranked_feature.similar_implementations:
                section += f"**Similar Implementations Found:**\n"
    ...
    def _generate_fork_statistics(self, fork_analyses: List[ForkAnalysis]) -> str:
        ...
        for fork_analysis in forks_by_features:
            ...
            last_activity = fork.last_activity.strftime('%Y-%m-%d') if fork.last_activity else 'Unknown'
    ...
    def _generate_analysis_metadata(self, metadata: Dict) -> str:
...
For questions about this analysis, please refer to the [Forklift documentation](https://github.com/forklift/forklift)."""
    ...
    def _generate_code_snippet(self, feature: Feature) -> str:
        ...
        commit_url = f"{feature.source_fork.repository.html_url}/commit/{main_commit.sha}"
