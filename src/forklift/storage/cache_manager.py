...
class CacheMonitoringMetrics:
    ...
    def __init__(self):
        ...
        self.last_warming: Optional[datetime] = None
...
class CacheManager:
    ...
    def __init__(
        self, 
        cache: Optional[ForkliftCache] = None,
        analysis_cache: Optional[AnalysisCacheManager] = None,
        ...
        self._warming_semaphore: Optional[asyncio.Semaphore] = None
            ...
            try:
                ...
                if config.warm_repository_metadata:
                    try:
                        ...
                        result["errors"].append(f"Failed to warm metadata for {repo_url}: {str(e)}")
                ...
                if config.warm_fork_lists:
                    try:
                        ...
                        result["errors"].append(f"Failed to warm fork list for {repo_url}: {str(e)}")
                ...
                if config.warm_recent_analyses:
                    try:
                        ...
                        result["errors"].append(f"Failed to warm analyses for {repo_url}: {str(e)}")
                
            except Exception as e:
                result["errors"].append(f"Failed to warm repository {repo_url}: {str(e)}")
            
            return result
    
    async def cleanup_cache(
        self, 
        cleanup_config: Optional[CacheCleanupConfig] = None
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if config.remove_expired:
                try:
                    ...
                    results["errors"].append(f"Failed to remove expired entries: {str(e)}")
            ...
            if config.remove_old_entries:
                try:
                    ...
                    results["errors"].append(f"Failed to remove old entries: {str(e)}")
            ...
            if current_size_mb > config.max_cache_size_mb:
                try:
                    ...
                    results["errors"].append(f"Failed to reduce cache size: {str(e)}")
            ...
            if config.vacuum_after_cleanup:
                try:
                    ...
                    results["errors"].append(f"Failed to vacuum database: {str(e)}")
            ...
            results["errors"].append(f"Cache cleanup failed: {str(e)}")
    ...
    async def schedule_maintenance(
        ...
        try:
            ...
            results["errors"].append(f"Cache maintenance failed: {str(e)}")
