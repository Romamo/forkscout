...
class TestRepositoryDisplayService:
    ...
    def test_calculate_activity_status_active(self):
        ...
        repo = Mock()
        repo.pushed_at = datetime.utcnow().replace(tzinfo=timezone.utc)
        result = self.service._calculate_activity_status(repo)
    ...
    def test_calculate_activity_status_moderate(self):
        ...
        from datetime import timedelta
        repo = Mock()
        repo.pushed_at = (datetime.utcnow() - timedelta(days=60)).replace(tzinfo=timezone.utc)
        result = self.service._calculate_activity_status(repo)
    ...
    def test_calculate_activity_status_stale(self):
        ...
        from datetime import timedelta
        repo = Mock()
        repo.pushed_at = (datetime.utcnow() - timedelta(days=200)).replace(tzinfo=timezone.utc)
        result = self.service._calculate_activity_status(repo)
        assert result == "stale"
    ...
    def test_calculate_activity_status_inactive(self):
        ...
        repo = Mock()
        repo.pushed_at = datetime.utcnow().replace(year=datetime.utcnow().year - 2).replace(tzinfo=timezone.utc)
        result = self.service._calculate_activity_status(repo)
        assert result == "inactive"
    ...
    def test_calculate_fork_activity_status_no_pushed_date(self):
        ...
        repo = Mock()
        repo.created_at = datetime.utcnow().replace(tzinfo=timezone.utc)
        repo.pushed_at = None
        result = self.service._calculate_fork_activity_status(repo)
        assert result == "No commits"
    ...
    def test_display_forks_table_empty(self):
        ...
        assert "[yellow]No forks found.[/yellow]" == call_args
